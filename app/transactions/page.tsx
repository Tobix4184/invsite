...
const META: Record<string, { icon: typeof Gift; tint: string; bg: string; sign: string }> = {
  deposit: { icon: ArrowDownLeft, tint: "text-success", bg: "bg-success/12", sign: "+" },
  withdrawal: { icon: ArrowUpRight, tint: "text-gold", bg: "bg-gold/12", sign: "-" },
  earning: { icon: TrendingUp, tint: "text-success", bg: "bg-success/12", sign: "+" },
  referral: { icon: Users, tint: "text-primary", bg: "bg-primary/12", sign: "+" },
  bonus: { icon: Gift, tint: "text-gold", bg: "bg-gold/12", sign: "+" },
  investment: { icon: Receipt, tint: "text-primary", bg: "bg-primary/12", sign: "-" },
  refund: { icon: ArrowDownLeft, tint: "text-success", bg: "bg-success/12", sign: "+" },
  credit: { icon: ArrowDownLeft, tint: "text-success", bg: "bg-success/12", sign: "+" },
  debit: { icon: ArrowUpRight, tint: "text-destructive", bg: "bg-destructive/12", sign: "-" },
}
    ...
    <div className="min-h-screen pb-28">
      ...
      <main className="mx-auto flex max-w-md flex-col gap-4 px-4 py-5 animate-fade-up">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black tracking-tight">Transaction History</h1>
        </div>
          ...
          <section className="card-glass overflow-hidden rounded-3xl">
            {txns.map((t, i) => {
              ...
              return (
                <div
                  key={t.id}
                  className={`flex items-center gap-3 p-4 transition-colors hover:bg-surface/50 ${i !== txns.length - 1 ? "border-b border-border/60" : ""}`}
                >
